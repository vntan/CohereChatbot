...
export default function HistoricalChats({
  isOnAddChatName,
  addChatName = { chatID: "", chatName: "", createTime: "" },
  updateChatName = { chatID: "", rename: "" },
  onChatClick = () => {},
  onCreateChat = () => {},
  onDeleteChat = () => {},
  onEditChat = () => {},
    ...
    if (updateChatName.chatID && updateChatName.chatID !== "") {
      if (updateChatName.rename && updateChatName.rename !== "") {
        const index = chatList.findIndex(
          (item) => item.chatID === updateChatName.chatID
        );
  ...
  return (
    <>
      <div
        class={`${isOnAddChatName && styles["loading-cursor"]} ${
          styles["chat_new"]
      ...
      <div class={`${styles["history_panel"]} flex-grow-1`}>
        {
          ...
          chatList.map((nameChatObj, pos) => {
            ...
            const isLoading =
              (isProcessDelete && isIDChatDelete) ||
              (isProcessEdit && isIDChatEdit);
            return (
              <>
                <div
                  onClick={(e) => {
                  ...
                  class={`${styles["chat_item"]} ${
                    isLoading ? styles["loading-cursor"] : ""
                  } chat_item_edit d-flex align-items-center`}
                ...
                >
                  ...
                  {!isEdit ? (
                    ...
                    <>
                      <input
                        type="text"
                        class={`${styles["inputEditNameChat"]} flex-grow-1`}
                        onChange={(e) => setInputChatName(e.target.value)}
                        value={inputChatName}
                        disabled={isProcessEdit}
                      />
